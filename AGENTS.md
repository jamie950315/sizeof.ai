...
## Current evidence release (2026-09-08)

- Testnet Worker `0b011b8a-384f-4b1c-af63-d585d8b9d314`; docs Worker `e2d05767-87bd-4fa6-8484-22c42d779c76`. This release supersedes earlier single-file/cache/backup limitations above. Production is unchanged.
- `/status` and `/api/status` expose allowlisted JP/US health, freshness, counts, generation alignment and backfill evidence with bounded checks. Both local databases served 3,055,452 names at verification; the San Jose public ingress remains unreliable from Cloudflare and is correctly shown as unreachable/unknown. No ingress migration was performed. Initial backfill completion does not prove complete Hub coverage; historical completion timestamps remain null. JP publisher and both APIs run the new health fields.
- `/compatibility` separates OS/backend/format/family evidence with primary sources, reviewed dates and explicit unknown states; it never guarantees arbitrary model compatibility. `/context` validates integer token allocations for seven categories, computes remaining/overflow and supports share/export without tokenizing user text.
- `/benchmarks` imports local detailed vLLM or generation-only llama-bench JSON through preview and explicit confirmation. Limits remain 2 MB/200 rows; llama requires explicit zero prompt/depth and raw samples; vLLM requires coherent per-request arrays. Aggregate-only/mixed results are rejected, unavailable metrics remain unknown, and response/error content is not retained.
- `/deploy` now accepts complete verified GGUF shard manifests and pins all filenames to one revision before opening the first shard. v3 plan links require a revision and complete list (64 files, 6,000-character manifest, 8,192-character URL limits); older links reject shard fields. Run backups write v3 and read v1/v2. Artifact records preserve actual file sizes and reject mismatched lists/totals. Model KV namespace is v5 so pre-manifest cached responses cannot bypass these facts.
- `/model-changes` and `/api/model-changes` verify current public visibility before comparing immutable revisions, complete sibling manifests, selected config facts and license declarations. Upstream JSON is bounded to 1 MB and listings to 1,000 files; incomplete/invalid results fail explicitly. Missing blob evidence stays unknown. Full config values are compared before display truncation; license declarations are not a legal-text diff. Saved runs link to this checker.
- Docs remain 17 guides with tool links and updated shard/import instructions. Verified 868 JS/TS tests plus 61 Python tests, Node 24 build/typecheck, local Worker smoke and live desktop/mobile flows: status, compatibility, context overflow, real two-shard command generation, revision LICENSE addition and synthetic import confirmation. Both public sites serve the same final asset build. Production homepage hash remains `c6900a49382143aede43fcdf74203f15ad073211232d0286afc1dbddb1d7ddeb`. No weights were downloaded and no real inference or hardware benchmark was performed.

## Current San Jose ingress repair (2026-09-08)

- Testnet Worker `4626f99f-9b24-49ab-a3d5-131319e1e295` uses `https://sizeof-search-us.0ruka.dev` for the US search source. Production and docs were not deployed. This supersedes the public-US-route warning in the evidence release above.
- Root diagnosis: the configured Tailscale Funnel public IPs timed out before TCP connection from JP/US and the Worker failed its health check, while MagicDNS/private requests and localhost were healthy. Public-relay failure is isolated from database health; its provider/network-level cause remains unproven. DERP wildcard port 443 also produces Tailscale listener warnings and was left untouched.
