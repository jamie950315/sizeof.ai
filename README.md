...
- A full 40-character model commit enables a separate `hf download --revision ... --local-dir ...` step. The engine then opens that local file/directory. Repository and revision get separate portable directories. Pinned GGUF is single-file only; MLX/vLLM downloads the full repository at that revision. Runtime, dependency, driver and hardware versions are **not** pinned or installed by the site. Stop if download fails; never silently replace the commit.
- Pinned plan URLs use `v=2`; legacy unpinned links remain `v=1`. A revision on a v1 URL is rejected so an old client cannot silently treat a pinned plan as current-branch. Deployment backups write version 2 and read valid legacy version 1 data. Old clients reject the newer backup instead of stripping its revision.
