...
export default function StartPage() {
  ...
  return <main className="platform-main">
    ...
    <section aria-labelledby="platform-tools"><div className="platform-section-title"><h2 id="platform-tools">One workspace. Nine useful entry points.</h2><span>NO ACCOUNT REQUIRED</span></div><div className="platform-tool-grid">{tools.map((tool) => <a className="platform-tool" href={tool.href} key={tool.href}><span>{tool.number} / {tool.label}</span><h3>{tool.name} <b aria-hidden="true">↗</b></h3><p>{tool.description}</p></a>)}</div></section>
