    ...
    {error ? <section role="alert" className="platform-alert"><p>{translate(error)} No diagnostic advice is shown for this invalid link.</p><button onClick={() => { setError(''); choose({ ...DEFAULT_DIAGNOSTIC }); window.history.replaceState(null, '', '/troubleshoot') }}>Reset invalid link</button></section> : <>
      ...
      <div className="trouble-workspace">
        <section className="trouble-controls" aria-label="Diagnostic selection">
          ...
          <label>Failure stage<select value={selection.stage} onChange={e => { const stage = e.target.value as Stage; choose({ ...selection, stage, symptom: PATHS[stage][0].id }) }}>{STAGES.map(([id, title]) => <option value={id} key={id}>{translate(title)}</option>)}</select></label>
          <label>Symptom<select value={selection.symptom} onChange={e => choose({ ...selection, symptom: e.target.value })}>{PATHS[selection.stage].map(p => <option value={p.id} key={p.id}>{translate(p.title)}</option>)}</select></label>
        ...
        <section className="trouble-evidence" aria-label="Evidence questions">
          ...
          {path.questions.map((q, i) => <label key={`${selection.stage}-${selection.symptom}-${i}`}>{translate(q)}<select value={answers[i]} onChange={e => setAnswers(prev => prev.map((a, index) => index === i ? e.target.value : a))}><option value="">Choose an observation</option><option value="yes">Yes / confirmed</option><option value="no">No / not confirmed</option><option value="unknown">Unknown — needs checking</option></select></label>)}
          {ready ? <div className="trouble-next" aria-live="polite"><span className="platform-eyebrow">NEXT SAFE CHECK · NOT A VERIFIED FIX</span><h3>{translate(path.title)}</h3><p>{translate(path.check)}</p>{answers.includes('unknown') && <p className="trouble-unknown">Some evidence is still unknown. Start with the unanswered checks before attempting a change.</p>}{answers.includes('no') && <p className="trouble-unknown">At least one item is not confirmed. Verify that prerequisite before treating a suggested cause as likely.</p>}<h3>Possible causes</h3><p>{translate(path.causes)}</p><h3>On {selection.os}</h3><p>{translate(SYSTEM_CHECKS[selection.os])}</p><div className="trouble-stop"><strong>When to stop</strong><p>{translate(path.stop)}</p></div><a href={`/docs/${path.doc}`}>Read the related guide →</a></div> : <p className="trouble-pending">Answer both questions, including “Unknown” when needed, to reveal the next check.</p>}
        </section>
      </div>
