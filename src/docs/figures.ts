...
export function docFigures(slug: string, locale: Locale = getActiveLocale()): DocFigure[] {
  ...
  if (slug === 'hardware') return [{ src: locale === 'en' ? '/assets/docs/memory-pools.svg' : `/assets/docs/memory-pools.${locale}.svg`, width: 1100, height: 620,
