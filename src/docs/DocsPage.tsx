...
export default function DocsPage({ basePath, pathname }: { basePath?: string; pathname?: string } = {}) {
  ...
  return <main className="docs-shell">
    <aside className="docs-sidebar" aria-label="Documentation navigation">
      <a href={base || '/'} className="docs-wordmark"><BookOpen size={17} /> FIELD GUIDE <span>01—{docsArticles.length}</span></a>
      ...
      <nav id="docs-guides" className={navOpen ? 'docs-guides docs-guides-open' : 'docs-guides'} aria-label="Guides">{categories.map((category) => {
        ...
        return <div className="docs-nav-group" key={category}><h2>{translate(category)}</h2>{group.map((item) => <a key={item.slug} href={docHref(item.slug, base)} aria-current={article?.slug === item.slug ? 'page' : undefined}>{translate(item.title)}</a>)}</div>
      })}</nav>
    ...
    <div className="docs-main">
      {slug && !article ? <section className="docs-missing"><span className="docs-eyebrow">404 / GUIDE NOT FOUND</span><h1>This page is not in the field guide.</h1><p>The link may be incorrect. Browse the guides or search for a topic.</p><a href={base || '/'}>Back to documentation <ArrowRight size={16} /></a></section> : article ? <>
        <header className="docs-article-header"><a href={base || '/'} className="docs-eyebrow">FIELD GUIDE</a><span className="docs-eyebrow"> / {translate(article.category)}</span><h1>{translate(article.title)}</h1><p>{translate(article.description)}</p><div className="docs-meta"><span>{translate(article.level)}</span><span>Reviewed {DOCS_REVIEWED_AT}</span><span>{Math.max(1, Math.ceil(article.sections.map((s) => s.paragraphs.join(' ') + (s.bullets ?? []).join(' ')).join(' ').split(/\s+/).length / 200))} min read</span><a href={`https://docs.sizeof.ai/${article.slug}.md`}>Read as Markdown ↗</a></div></header>
        {docFigures(article.slug).map((figure) => <figure className="docs-figure" key={figure.src}><img src={figure.src} width={figure.width} height={figure.height} alt={translate(figure.alt)} loading="lazy" decoding="async" /><figcaption>{translate(figure.caption)} <a href={figure.src} target="_blank" rel="noreferrer">Open full-size image ↗</a></figcaption></figure>)}
        ...
        {evidenceTools[article.slug] && <p className="docs-workflow-link">{evidenceTools[article.slug].map(([label, href]) => <a key={href} href={`${platformOrigin}${href}`} style={{ marginRight: 18 }}>{translate(label)} →</a>)}</p>}
